...
class ListCommand(Command):
    ...
    def execute(self, user: User, parameters: List[str]):
        try:
            self.raw_messages.raw(user, "321", user.nickname)
            for chanid in self._yield_channels():
                chanusers = str(len(chanid._users) - len(chanid._watch))
                if chanid.MODE_auditorium and user.nickname.lower() not in self.self.server_context.operator_entries and isOp(
                        user.nickname.lower(), chanid.channelname) == False:
                    chanusers = str((len(chanid._op) + len(chanid._owner)))

                if chanid.visible_in_list():
                    if user.nickname.lower() in chanid._users or user.nickname.lower() in self.server_context.operator_entries:
                        self.raw_messages.raw(user, "322", user.nickname,
                    ...
                    if parameters[0] == "LISTX" and len(parameters) == 2:
                        ...
                        if "<" in parameters[1]:
                            ...
                            try:
                                ...
                                # TODO don't swallow exceptions, I suspect index error here
                                # tidy this up
                                pass
        ...
        self.raw_messages.raw(user, "323", user.nickname)